...
public class AvatarSystem : MonoBehaviourSingleton<AvatarSystem>
{
    ...
    private void SetBodyParts()
    {
        ...
        _bodyParts[EYES].masks = eyesMasks;
    ...
    public GameObject SetBaseMesh(bool isMale, GameObject avatar, Transform parent = null)
    {
        ...
        if (isMale)
            newAvatar = Instantiate(baseMale, parent);
        else
            newAvatar = Instantiate(baseFemale, parent);
        ...
        newAvatar.transform.SetAsFirstSibling();
        SetBones(newAvatar);
    ...
    public void SetAvatar(GameObject avatar, int? hair, int upper, int lower, int feet, int? facialHair, int? earring, int? mask, int? hat, int eyes, int mouth, int eyebrows, int skinColor, int hairColor, int eyesColor)
    {
        ...
        SetEyebrowsColor(hairColor, avatar);
        SetFacialHairColor(hairColor, avatar);
    ...
    public int GiveMeBodyPartRandomBySex(bool isMale, int bodyPart, bool isNulleable)
    {
        GameObject[] parts;
        switch (bodyPart)
        {
            ...
            case MASK:
                ...
                break;
            case FACIAL_HAIR:
                parts = facialHairMeshes;
                break;
            case EYES:
                return Random.Range(0, eyesTextures.Length);
            case EYEBROWS:
                return Random.Range(0, eyebrowsTextures.Length);
            case MOUTH:
                return Random.Range(0, mouthTextures.Length);
            default:
                throw new Exception($"Body part {bodyPart} not found.");
        }

        return GiveMeRandomBySex(isMale, parts, isNulleable);
    }

    public int GiveMeRandomSkinColor()
    {
        return Random.Range(0, skinMaterials.Length);
    }

    public int GiveMeRandomEyesColor()
    {
        return Random.Range(0, eyesColors.Length);
    }

    public int GiveMeRandomHairColor()
    {
        return Random.Range(0, hairColors.Length);
    }
