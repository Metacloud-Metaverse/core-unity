...
public static class NetworkManagerExtensions
{
	...
	public static void RegisterHandler<T, U>(bool isServer) where T : GameMessageBase<U>, new() where U : struct, NetworkMessage
	{
		var message = Activator.CreateInstance<T>();

		if (!isServer)
		{
			NetworkClient.ReplaceHandler(new Action<NetworkConnection, U>(message.PreProcess));
		}
		else
		{
